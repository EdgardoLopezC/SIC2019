	...
	if request.method=='POST':
		...
		if action =='nuevo':
			...
			if periodo1.fecha_inicio == periodo1.fecha_final :
				mensaje='No puede ingresar la misma fecha inicial y final'
			periodo = PeriodoContable.objects.all().order_by('-id')
			context={
				'periodo':periodo,
				'mensaje':mensaje,
...
def reg_transaccion(request, periodo_id):
	...
	if request.method =='POST':
		...
		if action == 'insert':
			...
			if salHaber == salDebe :
				text='comprobado saldo deudor igual al saldo acreedor'
			elif salDebe > salHaber :
				text ='saldo deudor mayor al saldo acreedor, no cuadra'
			elif salDebe <salHaber :
				text='saldo deudor menor al saldo acreedor, no cuadra'
		elif action=='ajuste' :
			pass
		elif action =='estados':
			pass
